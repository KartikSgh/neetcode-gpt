...
class MultiHeadedSelfAttention(nn.Module):
    ...
    def __init__(self, embedding_dim: int, attention_dim: int, num_heads: int):
        ...
        self.heads = nn.ModuleList([self.SingleHeadAttention(embedding_dim, head_size) for i in range(num_heads)])
    ...
    def forward(self, embedded: TensorType[float]) -> TensorType[float]:
        ...
        for layer in self.heads:
            attention_scores.append(layer(embedded))
        con_attn = torch.concat(attention_scores, dim=2)
        final_attn = self.output_projection(con_attn)
        return torch.round(final_attn, decimals=4)
