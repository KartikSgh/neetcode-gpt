...
class GPT(nn.Module):
    ...
    def __init__(self, vocab_size: int, context_length: int, model_dim: int, num_blocks: int, num_heads: int):
        ...
        self.word_embd = nn.Embedding(vocab_size, embedding_dim=model_dim)
        self.position_embd = nn.Embedding(context_length, embedding_dim=model_dim)
        ...
        self.final_projection = nn.Linear(model_dim, vocab_size)
    ...
    def forward(self, context: TensorType[int]) -> TensorType[float]:
        ...
        embd = self.word_embd(context) + self.position_embd(torch.arange(self.context_length))
        attention = self.transformer_blocks(embd)
        final = self.layernorm(attention)
        output = self.final_projection(final)
